...
- **SSH 连接管理** - 密码/密钥认证，连接池自动复用，私钥支持内容和文件路径两种方式
...
- **私钥文件路径支持** - `privateKey` 参数现在同时支持私钥内容和文件路径
  - 之前必须粘贴整个私钥内容，现在可以直接传文件路径如 `/path/to/id_rsa.pem` 或 `~/.ssh/id_rsa`
  - 自动识别格式：以 `-----BEGIN` 开头视为私钥内容，否则视为文件路径
  - 支持 `~` 家目录展开
